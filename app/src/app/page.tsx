...
export default function HomePage() {
  const [jobDescription, setJobDescription] = useState('Seeking GoLang dev (2+ yrs) to write/test backend code, build scalable services, solve complex problems. Must have 5+ yrs experience, good English, team spirit, and strong problem-solving skills.')
        ...
        <div className="mt-6 space-y-4">
          {results.map(candidate => (
            <div key={candidate.id} className="border p-4 rounded shadow">
              <h2 className="text-lg font-semibold">{candidate.name} ({candidate.score})</h2>
